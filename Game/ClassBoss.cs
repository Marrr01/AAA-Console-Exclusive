...
internal class ClassBoss : ClassGameObject
{
    ...
    public ClassBoss(Dictionary<int, string> Tags, char[,,] FieldOfAction)
    {
        Skin = new char[FieldOfAction.GetLength(0),2,2];
    ...
    public void Shooting(Dictionary<int, string> Tags, List<ClassEnemyPlasma> EnemyPlasmaList)
    {
        if (!Removed)
        {
            for (int i = 1; i < Skin.GetLength(0) - 1; i++)
            {
                ...
                {
                    EnemyPlasmaList.Add(new ClassEnemyPlasma(Tags, i, 1));
                }
            }
        }
    }

    public void WriteHP(char [,,] FieldOfAction, List<ClassBoss> BossList)
    {
        if (BossList.Count != 0)
        {
            Console.Write('|');

...
internal class ClassEnemyUnit : ClassGameObject // базовый класс для вражеских кораблей
{
        ...
        if (!Removed)
        {
            if (TickCounterShoot == RateOfFire)
            {
                for (int i = 1; i < Skin.GetLength(0); i++)
                {
                    if (i % 2 == 0)
                    {
                        EnemyPlasmaList.Add(new ClassEnemyPlasma(ref Tags, PosX + i - 1, PosY + Skin.GetLength(1) - 1));
                    }
                }
                TickCounterShoot = 0;
            }
    ...
    public void ScoreAndLoot(ref Dictionary<int, string> Tags, ref int Score, ref List<ClassHealthBuff> HealthBuffList)
    {
        if ((HealthPoints <= 0) && (!Removed))
        {
            ...
            if (Randomizer.Next(1, 100) <= BuffDropChance) { HealthBuffList.Add(new ClassHealthBuff(ref Tags, PosX + (Skin.GetLength(0) / 2), PosY)); }
        }
    }
}
